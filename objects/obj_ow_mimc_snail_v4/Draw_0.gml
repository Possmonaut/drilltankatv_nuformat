...
if(state = 6)
{
	...
	if(dTimer > dTime)
	{
		...
		{
			var xxscale = ((dTimer-dTime)* 0.0033) *2.5;
			var yyscale = ((dTimer-dTime)* 0.0033) *2.5;
...
var startPX = x +16+ lengthdir_x(24,myDir);
var startPY = y +16+ lengthdir_y(24,myDir);
...
if(state = 3 || state = 4)
{
	...
	if(partGen > 5)
	{
	part_type_orientation(global.footPart,myDir+90,myDir+90,0,0,false);
	part_particles_create(global.P_System,x+16,y+16,global.footPart,1);
	partGen = 0;
	}
}

draw_sprite_ext(sp_mimic_foot,0,x+16,y+16,buttScale,buttScale,spDir+90,c_white,1); //change scale based on foot animations 
draw_sprite_ext(sp_mimic_hand, armStp, startPX, startPY, pincerL.image_xscale, pincerL.image_yscale, spDir, c_white, 1);
